...
```{r}
covid_county_monthly_death_rate <- read_csv("data/covid_county_monthly_case_rate.csv")
```
...
# Q: Which Virginia county had the LOWEST case rate on August 30? What was the case rate (cases per 1000)?
# A: Bath 
...
# A: 10 counties? Listed below? I did not get the same code that is in the video.
...
# A: 3 Galax City, Emporia City, Northhampton
...
```{r}
covid_county_jun_aug_pct_change <- covid_county_monthly_case_rate %>%
  select(county, state, d_2020_07_30, d_2020_08_30) %>%
  filter(state == "Maryland") %>%
  mutate(jun_aug_pct_change = (d_2020_08_30 - d_2020_07_30)/d_2020_07_30) %>%
  arrange(desc(jun_aug_pct_change))
...
# A: Baltimore City and Worcester County
...
```{r}
covid_county_may_aug_pct_change <- covid_county_monthly_case_rate %>%
select(county, state, d_2020_05_30, d_2020_08_30) %>%
filter(state == "Texas") %>%
mutate(may_aug_pct_change = (d_2020_08_30 - d_2020_05_30)/d_2020_05_30) %>%
arrange(desc(may_aug_pct_change))
```

### Exercise 3


Which Virginia County had the lowest case rate on Aug. 30?
...
The answer to above question is Bath!
