...
group_by(state)  %>%
  summarise(deaths_in_average_county = mean(deaths)) %>% arrange(desc(deaths_in_average_county))
...
# A: write the answer here Delaware
...
How many counties are there in Tennessee? Answer: 95
```{r}
counties_per_state <- covid_county %>%
  group_by(state) %>%
  summarise(total_counties = n()) %>%
