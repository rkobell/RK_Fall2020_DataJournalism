...
arcos_md_baltimorecity_pharmacies <- arcos_md_baltimorecity %>% select(buyer_name)
```
...
arcos_wv_mingo_trimmed_new <- arcos_wv_mingo %>%
  select
...
```{r}
arcos_md_baltimorecity_trimmed <- arcos_md_baltimorecity %>% select (buyer_name, buyer_dea_no, buyer_city, total_dosage_unit, total_records)
arcos_wv_mingo_trimmed <- arcos_wv_mingo %>%
...
```{r}
#Answer New Care Home Health Services

arcos_md_baltimorecity_sorted <- arcos_md_baltimorecity_trimmed %>% arrange(desc(total_dosage_unit))
...
```{r}
# Answer to question 1: New Care Health Services, 6,133,600 shipments
...
```{r}
# Answer to question 2: Julian Jakobovits, 5 pills
