...
Q: Which state had the most total covid cases on August 30?
...
# Write the answer in a comment
# Q: Which state has the most average highest number of cases per county? DC.
# A: 
```
...
Q: Which state has the most counties in our data? How many? Texas, 251.
...
```{r}
min_max_deaths <- covid_county %>%
  group_by(state) %>%
  summarise(max_death_state = max(deaths),
  min_death_state = min(deaths),
...
group_by(state) %>%
summarize(averages_county_deaths = mean(deaths)) %>%
arrange(desc(averages_county_deaths))
averages_county_deaths_by_state
```

It's okay if you don't get through alll of them, just get through as many as you can.  
 
* Q: Which state had the highest total number of deaths? New York
* Q: Which state had the lowest total number of deaths?Alaska
* Q: Which state had the highest average number of deaths per county? What was that number? New Jersey 758.9
* Q: Which state had the lowest average number of deaths per county? What was that number? Alaska
