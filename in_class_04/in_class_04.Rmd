---
title: "JOUR472/772 in_class_04 | heat analysis, grouping and summarizing"
...
# Write the answer in a comment
# Q: What is the average heat index value on each day inside of Stephanie's house? Which day had the highest?
# A: 7/20/2019 had the highest at 101.1.
...
# Write the answer in a comment
# Q: During the seven day heat wave, what was the lowest recorded heat index value in Stephanie's house?  What was the highest heat index value, and on what day did it occur?
# A: lowest was 88.8.The highest was 115 degrees, and that was July 19.
...
```{r}
michael_day_hourly_averages <- michael_day_hourly_averages%>%
mutate(date = date(date_hour))%>%
select(date, everything()) %>%
filter(date >= "2019-07-16") %>%
filter(date <= "2019-07-22")
...
```{r}
michael_daily <- michael_day_hourly_averages %>%
  group_by(date) %>%
  summarise(heat_index_max=max(mean_indoor_heat_index),
              heat_index_min=min(mean_indoor_heat_index),
              average_heat_index = mean(mean_indoor_heat_index),
              count = n()
  )
michael_daily
```
...
* Q: What was the highest heat index value measured in Michael's house? On what day did it occur? 114.6 occurred on 7/18/2018

* Q: The heat index never fell below what value during the period we measured? 
never fell below 96 degrees.

* Q: What was the average heat index value on each day during the period measured?
The average daily over the 5 day period is 105.2 (added and divided by 5.)

* Q: Is there anything concerning the number of hourly readings on each day that could affect the results you're getting?

Looking at the hourly readings, the temperatures are much higher at night.Also, this talks about average and the other table talks about mean. Mean is the middle, but it is not the same as adding them all and getting an average.
